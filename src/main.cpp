...
int main() {
  ...
  GLFWwindow* window =
      glfwCreateWindow(width, height, "mushroom 3d pyramid", NULL, NULL);
  ...
  /* 	std::string parentDir =
     (fs::current_path().fs::path::parent_path()).string(); std::string texPath
     = "/Resources/YoutubeOpenGL 6 - Textures/"; */

  // Texture
  /* Texture popCat((parentDir + texPath + "pop_cat.png").c_str(),
  GL_TEXTURE_2D, GL_TEXTURE0, GL_RGBA, GL_UNSIGNED_BYTE);
  popCat.texUnit(shaderProgram, "tex0", 0); */

  // Original code from the tutorial
  Texture popCat("mushroom.png", GL_TEXTURE_2D, GL_TEXTURE0, GL_RGBA,
                 GL_UNSIGNED_BYTE);
  popCat.texUnit(shaderProgram, "tex0", 0);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    cout << crntTime << "time" << endl;
    if (crntTime < 6 && crntTime > 5) break;
